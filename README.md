...
- 読み取り専用デモモード: 環境変数 `DEMO_READ_ONLY=true` で全書き込み API（POST / PATCH / DELETE）を 405 にし、閲覧用に公開する場合の設定として使います
...
- 閲覧用に外部公開して運用する場合は読み取り専用モード（`DEMO_READ_ONLY=true`）を使用し、外部からの実データ投入は受け付けません
