...
test('モバイルナビを開くと閉じるボタンへフォーカスを移す', async ({ page }) => {
  await openMobileNavigation(page)

  await expect(page.locator('aside').getByRole('button', { name: 'メニューを閉じる' })).toBeFocused()
})

test('モバイルナビを開いていると背景を操作対象から外す', async ({ page }) => {
  await openMobileNavigation(page)

  const openButtonReceivedFocus = await page.getByRole('button', { name: 'メニューを開く' }).evaluate((button) => {
    button.focus()
    return document.activeElement === button
  })

  expect(openButtonReceivedFocus).toBe(false)
})
