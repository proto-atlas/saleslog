...
## 静的UIデモと画面資料（2026-06-28 実施）

| 方法 | 対象 | 結果 |
|---|---|---|
| `npm.cmd run typecheck` | frontend TypeScript | exit 0 |
| `npm.cmd run lint` | frontend ESLint | exit 0 |
| `npx.cmd vitest run src/api/staticDemo.test.ts --project=unit --browser.enabled=false` | 静的UIデモ用API応答。固定ユーザー、顧客一覧、顧客登録、AgentイベントSSE | **1 file / 3 tests passed** |
...
| Cloudflare Pages + Playwright | `https://saleslog-demo.pages.dev/`、`/customers`、`/customers/1?tab=agent`、デモバナー表示、Agent提案表示、承認操作 | HTTP 200。Playwrightで承認トースト「承認して保存しました」を確認。ブラウザconsole error 0件 |
...
frontend 全体E2Eの直近記録は下記の「E2E DB準備位置の検証」セクションに残す。
...
## worker timeout競合・Anthropic標準設定の検証（2026-06-20 実施）
...
## Agent・認証cache・文書表現の検証（2026-06-20 実施）
...
| `rg` による配布対象の語句確認 | 環境依存パス表記、secret形状、合成データ名の残存 | ヒットなし |

## 承認編集version必須化・E2Eの検証（2026-06-21 実施）
