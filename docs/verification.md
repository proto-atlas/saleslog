...
frontend はこの修正で変更していない。直近の全体E2E記録は下記の「E2E DB準備位置の再確認」セクションに残す。
...
- この確認で発覚した 2 件を修正済み:
  - 顧客詳細の活動履歴 API に sales スコープが漏れ、他人の記録が表示される（開くと 404）→ 一覧 API と同じ「自分の記録のみ」に統一し、pytest に履歴スコープの検査を追加
  - 活動記録の編集保存成功後の遷移が未保存確認に誤ブロックされる（認証と無関係の潜伏バグ）→ 保存完了 state で確認を解除してから遷移する方式に修正し、「保存後は確認なしで遷移する」E2E を追加（修正前コードで failed になることを確認したうえで修正後 passed）
