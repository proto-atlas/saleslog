...
## 検証
- [x] テスト: `docs/verification.md` の認証・認可項目で、未登録 subject、role 別アクセス制御、manager 限定 API を検証
- [x] 手動確認: `AUTH_MODE=clerk` でサインイン後の `/api/me` と画面遷移を確認
- [x] 公開表現: README と API 応答に認証 provider の subject 生値を返さない
